      ...
      .then(data=>{
         ...
         if(data.error){
          M.toast({html: data.error, classes:"#388e3c green darken-1"})
             ...
             M.toast({html:"created post success", classes:"#388e3c green darken-1"})
             history.push('/')
         }
  }).catch(err=>{
      console.log(err)
...
const postDetails = ()=>{
  ...
  data.append("cloud_name","cloudykhushboo")
  fetch("https://api.cloudinary.com/v1_1/cloudykhushboo/image/upload",{
    method:"post",
    body:data
  })
  .then(res=>res.json())
